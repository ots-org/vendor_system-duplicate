...
public class StatusListActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra("myRequests")){
            getEnterpriseSearchResults(AVAILABLE_TYPE.CANCEL);
        }
    ...
    private void getEnterpriseSearchResults(final AVAILABLE_TYPE type) {
                ...
                switch (type) {
                    case NEW:
                        ...
                        if (getIntent().hasExtra("myRequests")){
                            bundle.putString("status","NewRequest");
                        }else {
                    ...
                    case ASSIGNED:
                            ...
                            if (getIntent().hasExtra("myRequests")){
                                bundle2.putString("status","AssignedRequestToEmployee");
                            }else {
                    ...
                    case CLOSED:
                            ...
                            if (getIntent().hasExtra("myRequests")){
                                bundle3.putString("status","DoneDonation");
                            }else {
                        ...
                        break;
                    case CANCEL:
                        Fragment unsubscribedFagments = CustomerStatusListFragment.newInstance();
                        Bundle bundle4 = new Bundle();
                        if (getIntent().hasExtra("myRequests")){
                            bundle4.putString("status","cancelRequest");
                        }else {
                            bundle4.putString("status","Close");
                        }
                        unsubscribedFagments.setArguments(bundle4);
                        adapter.addFragment(unsubscribedFagments, "Cancel");
                        break;
                }
