...
import com.ortusolis.subhaksha.Activity.ApplicationSelectorReceiver;
...
//            checkout.setKeyID("rzp_live_4uHrzyoxasNhiq");
            checkout.setKeyID(razorpayKeyApiResponce);
...
//            checkout.setKeyID("rzp_test_oGbRsq49chxYFD");
            ...
            public void notifySuccess(String response, int statusCode) {
                try {
                    ...
                    if (obj.getString("responseCode").equalsIgnoreCase("200")) {
                        ...
                        ClipData clip = ClipData.newPlainText("donation", ""+sharedPreferences.getString("userFirstName", "")+" "+sharedPreferences.getString("userLastName", "")+" of "+ sharedPreferences.getString("userAddress1", "")+ " has donated  "+productName +" of worth "+getString(R.string.Rs) + dontaionAmount+" towards "+BeneficieryName.split("\\(")[0]+" using  subhaksha App \n https://play.google.com/store/apps/details?id=com.ortusolis.subhaksha \n We thank you for this contribution.");
        ...
        wss.postLoginVolley(Constants.addNewDonation, requestObject.toString(), new IResult() {
            ...
            public void notifySuccess(String response, int statusCode) {
                try {
                    ...
                    if (obj.getString("responseCode").equalsIgnoreCase("200")) {
                        ClipboardManager clipboard = (ClipboardManager) getSystemService(CLIPBOARD_SERVICE);
                        ClipData clip = ClipData.newPlainText("donation", ""+sharedPreferences.getString("userFirstName", "")+" "+sharedPreferences.getString("userLastName", "")+" of "+ sharedPreferences.getString("userAddress1", "")+ " has made donation "+getString(R.string.Rs) + dontaionAmount+" using subhaksha App \n https://play.google.com/store/apps/details?id=com.ortusolis.subhaksha \n We thank you for this contribution.");
                        clipboard.setPrimaryClip(clip);
//                        Toast.makeText(getApplicationContext(),"Copy successful,paste",Toast.LENGTH_SHORT).show();
                        new Thread(new Runnable() {
                            public void run(){
...
//                        Toast.makeText(getApplicationContext(),"payment Success",Toast.LENGTH_SHORT).show();
                    ...
                    razorpayKeyApiResponce=obj.getJSONObject("responseData").getString("razorPayKey");
...
//                Toast.makeText(getApplicationContext(), WebserviceController.returnErrorMessage(error), Toast.LENGTH_LONG).show();
    ...
    void DonationImage(final String text) throws IOException {
        ...
        sendIntent.putExtra(Intent.EXTRA_TEXT, text+" using subhaksha App \n https://play.google.com/store/apps/details?id=com.ortusolis.subhaksha \n We thank you for this contribution.");
