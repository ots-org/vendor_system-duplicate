...
public class OTSUserServiceImpl implements  OTSUserService{
	...
	public List<GetcartListResponse> getcartList(AddToCartRequest addToCartRequest) {
		...
		for(int i=0;i<cartList.size();i++) {
			...
			BigDecimal price= new BigDecimal(cartList.get(i).getProductPrice());
			BigDecimal itemCost=price.multiply(new BigDecimal(Quantity));
			totalCost=totalCost.add(itemCost);
			cartList.get(i).setTotalPrice(totalCost);
		}
		return cartList;
	}
	...
	public List<Map<String, Object>> getSellerDetails(GetSellerByLatAndLonRequest getSellerByLatAndLonRequest) {
		...
		return userServiceDAO.getSellerDetails(getSellerByLatAndLonRequest);
	}
