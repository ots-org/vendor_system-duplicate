...
public class ActiveDirectoryAuthentication {
	...
	public UserDetails getAdsUser(String username, String password, String validateUserName) throws AccountException, FailedLoginException {
    	...
    	try {
			...
			SearchResult searchRes = findAccountByAccountName( ctx,  BASE_NAME_VALID,  validateUserName);
			if(searchRes!=null) {
				Attributes attribs = searchRes.getAttributes();
	            if (null != attribs)
	            {
	                for (NamingEnumeration ae = attribs.getAll(); ae.hasMoreElements();)
	                {
	                    ...
	                    if(attributeID.equals("userPrincipalName")) {
	                    	for (Enumeration vals = atr.getAll();
	                                vals.hasMoreElements(); 
	                    			userDetails.setEmailId(vals.nextElement()+""));
	                    }
	            ...
	            userDetails.setAdsAuthentication(true);
			}else {
				userDetails.setAdsAuthentication(false);
			}
