...
public class ProductStockHistoryDaoImpl extends AbstractIptDao<OtsProductStockHistory, String> implements ProductStockHistoryDao{
	...
	public long getOtsProductStockAddition(Integer otsProductId,
		...
		System.out.println("otsProductId : "+otsProductId+
						   "OtsUsers : "+getProductStockListRequest.getRequestData().getUserId()+
						   "Date : "+getProductStockListRequest.getRequestData().getTodaysDate());
		try {
			...
			otsProductStockHistory = super.getEntityManager()
					.createQuery("from OtsProductStockHistory where  otsUsersId = ?1 and otsProductId = ?2 and otsProductStockAddDate = ?3  and (otsProductStockOrderId = null or otsProductStockOrderId='') ", OtsProductStockHistory.class)
